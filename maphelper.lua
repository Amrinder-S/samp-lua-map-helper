...
allshacks = {
		{"whetstone", -1215.2387695313 ,-2346.8029785156 ,2.2546253204346},
		{"bob", -466.09173583984, -2421.5434570313, 111.99334716797},
		{"leafy", -1117.7976074219, -1617.6395263672, 76.37393951416},
		{"leafy2", -1117.9248046875, -1623.1689453125, 76.37393951416},
		{"bb", 172.80473327637, -568.99896240234, 45.7668800354},
		{"fern", 934.63073730469, 2.1267583370209, 91.870513916016},
		{"pano", -756.51330566406, -133.21873474121, 65.828125},
		{"nr", 2351.6276855469, -648.80529785156, 128.0546875},
		{"bc", -202.3597869873, 1873.7658691406, 50.27156829834},
		{"arco", -792.65856933594, 2422.177734375, 157.19564819336},
		{"tr", -2849.1240234375, 2789.3544921875, 244.12812805176},
}
shackstable = {}
allgasstations = {
		{"1", 2116.1042480469, 917.75, 10.8203125},
		{"2", 2636.7407226563, 1107.2180175781, 10.812978744507},
		{"3", 615.73315429688, 1692.58984375, 6.9921875},
		{"4", -1328.1636962891, 2677.66796875, 50.0625},
		{"5", -2411.140625, 973.25305175781, 45.4609375},
		{"6", -1673.2181396484, 412.47570800781, 7.1796875},
		{"7", -2020.3010253906, 157.09661865234, 28.488561630249},
		{"8", -1607.0659179688, -2712.0632324219, 48.533473968506},
		{"9", 1939.5532226563, -1773.3299560547, 13.390598297119},
		{"10", -91.75537109375, -1168.8441162109, 2.4409954547882},
		{"11", 1380.6854248047, 461.15658569336, 20.058528900146},
		{"12", 657.10101318359, -567.59436035156, 16.3359375},
		{"13", -1469.6802978516, 1862.9739990234, 32.6328125},
		{"14", 2148.9091796875, 2746.1518554688, 10.8203125},
		{"15", 2201.4909667969, 2473.3303222656, 10.8203125},
		{"16", 1596.0406494141, 2200.4965820313, 10.8203125},
		{"17", 1006.7735595703, -938.689453125, 42.1796875},
		{"18", -2241.9426269531, -2558.7238769531, 31.921875}
	}
gasstations= {}
...
name ()
  ...
  sampRegisterChatCommand ( "shack" , shack )
  sampRegisterChatCommand ( "mhelp" , mhelp)
  sampRegisterChatCommand ( "ar" , area )
  sampRegisterChatCommand ( "loc" , zonename )
  sampRegisterChatCommand ( "gs" , gs )
  sampRegisterChatCommand ( "rem", rem)
  wait ( - 1 ) 
end
function rem()
local temp = removeWaypoint()
end

function name ()
 repeat wait(0) until isSampAvailable()
sampAddChatMessage("MapHelper Loaded.", 0x00DDDD)
sampAddChatMessage("/mhelp", 0x00DDDD)
end

 function area ( arg )
	local areaCalculated = calculateZoneEN(getCharCoordinates(PLAYER_PED))
	if areaCalculated then
		sampAddChatMessage(areaCalculated, 0x00DD00)
	else
		sampAddChatMessage( "You are in an interior", 0x00DD00)
	end
end

function mhelp ()
sampAddChatMessage("cmd: /ar" , 0x00DD00)
sampAddChatMessage("cmd: /gs { 1 to 18 }" , 0x00DD00)
sampAddChatMessage("cmd: /shack { whetstone, bob, bb, bc, arco, fern, pano, nr, tr} " , 0x00DD00)
end
--[[
function rem ()
for i = 90,1,-1 do
removeBlip(shackstable[i])
end
sampAddChatMessage("All shack locations removed." , 0x00DD00)
end
...
function remogs ()
for i = 90,1,-1 do
removeBlip(gasstations[i])
end
end
]]--
function gs ( arg )
	-- remogs ()
	for i, v in ipairs(allgasstations) do
	if v[1] == arg then
	gasstations[90] = placeWaypoint(v[2], v[3], v[4])
	--changeBlipColour(gasstations[90], 5)
	sampAddChatMessage("Location marked for GS" .. v[1] .. ".", 0x00DD00) end
		end
	end
	
function shack ( arg )
--	remo ()
	for i, v in ipairs(allshacks) do
	if v[1] == arg then
	shackstable[90] = placeWaypoint(v[2], v[3], v[4])
--	changeBlipColour(shackstable[90], 2)
	sampAddChatMessage("Location marked for " .. v[1] .. " shack.", 0x00DD00) end
		end
	end
...
zoneloc = {}	
function zonename(arg)
    for i, v in ipairs(streets) do
        if string.find(string.lower(v[1]), arg ) then
			zoneloc[300] = placeWaypoint(v[2], v[3], v[4])
            return sampAddChatMessage( "location marked for " .. v[1], 0x00DD00)
        end
    end
    return sampAddChatMessage( "Location not found.", 0x00DD00)
end